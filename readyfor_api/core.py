...
class ReadyForConnection(object):
    ...
    def call(cls, object_name, object_id, sub_object, method="GET", **kwargs):
        ...
        try:
            ...
            response = request(method="GET", url=query, params=kwargs)
            parser_class_snake = "{{object_name}}_{{sub_object}}_page_parser".\
                format(object_name=object_name, sub_object=sub_object)
            parser_class_camel = cls.inflicter.camelize(word=parser_class_snake)
            return cls.str_to_cls(parser_class_camel)(response.text)
        except:
            raise AccessException("some problem occurred when access ")

    @staticmethod
    def str_to_cls(class_name):
